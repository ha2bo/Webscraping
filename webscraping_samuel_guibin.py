...
for products_EBAY in soup.findAll("li",{"class" : "s-item"}):
    if i < number_of_products:
        ...
        product_prize_final.append((float(number_extract(product_prize_list[i])) + float(number_extract(product_shipping_list[i]))).__str__()) #save the value of the product (price + shipping)
